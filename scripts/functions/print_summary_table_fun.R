...
print_summary_table <- function(
    ...
    var_strata, # name of strata variable; "taste_diplos_AA"
    ...
    #p_type="descriptive", # P-value type; default = descriptive; T.test/ANOVA for continuous; Chi Square for Categorical
  ...
  dat_total <- data %>% 
    select(strata=var_strata, all_of(names(vars_to_summarize)), all_of(p_adjust_vars)) %>%
  ...
  summary_by_strata <- 
    ...
    if(is.numeric(var.total.dat$Var) == T) {
      ...
      if(p_print == T) {
        ...
        else {P_Ftest=round(anova(lm(formula(paste0(var, "~strata_ordered", "+", paste0(p_adjust_vars, collapse = "+"))), data=dat_total))$'Pr(>F)'[1], d_pval)}
        
        var.summary <- cbind.data.frame(var.summary, P_value = P_Ftest) 
    ...
    if(is.numeric(var.total.dat$Var) == F) {
      ...
      if(p_print == T) {
        ...
        var.summary <- cbind.data.frame(var.summary, P_value=c(P_ChiSq, rep("", length(var_lvls))))
      } else {var.summary <- var.summary}
