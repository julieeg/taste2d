...
plot_emm_dPC_signif <- dietPC_emm %>%
  ...
  ggtheme_fancy+theme(axis.text.x = element_text(angle=35, hjust=1), legend.position = "bottom") +
...
ggarrange(plot_bar_dPCttest, plot_emm_dPC_signif, align = "hv", nrow = 2, heights=c(.5,1.5))
```
...
make_pretty_lm(read.csv("../data/processed/analysis/EUR_lm_diplo_x_gluXfast_v4.csv")) %>%
...
emm_gluXfast_diet <- read.csv("../data/processed/analysis/EUR_emm_diplo_x_gluXfast_diet_v4.csv")
...
p_emm_gluXfast_diet <- emm_gluXfast_diet %>%
  ...
  scale_color_manual(values=palettes$nature_diplosAA_canonical_yrb, name="TAS2R38 \nDiplotype") +
  ggtheme_fancy + ylab("Marginal mean (95% CI) blood glucose, mmol/L") + 
