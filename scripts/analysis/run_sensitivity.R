...
m4_diet = paste0(m2_lifestyle, "+dietPC1+dietPC2+dietPC3+dietPC4+dietPC5+dietPC6+dietPC7+dietPC8+dietPC9+dietPC10")
...
tab_lm_bitter_rg <- do.call(rbind.data.frame, lapply(bitter_snps, function(snp) {
  ...
  print_lm_cont(exposure = snp, outcome = "glu", covariates = models.l[[i]], label=names(models.l)[i], 
           data = analysis)  })) 
}))
...
tab_lm_bitter_a1c <- do.call(rbind.data.frame, lapply(bitter_snps, function(snp) {
  do.call(rbind.data.frame, lapply(1:length(models.l), function(i) {
    print_lm_cont(exposure = snp, outcome = "hba1c_max", covariates = models.l[[i]], label=names(models.l)[i], 
                  data = analysis)  })) 
}))
...
tab_lm_bitter_gluXfast <- do.call(rbind.data.frame, lapply(bitter_snps, function(snp) {
  ...
  do.call(rbind.data.frame, lapply(1:length(models.nofast.l), function(i) {
    as.data.frame(print_lm_cont(exposure = snp, outcome = "glu", covariates = models.nofast.l[[i]], 
