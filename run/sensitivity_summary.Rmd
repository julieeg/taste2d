...
```{r}
m_diet_all <- paste0("dietPC",1:24,collapse="+")

make_pretty_lm(rbind.data.frame(
  print_lm(exposure = "taste_diplos", outcome = "glu", label="Glucose ~ All Diet Patterns", covariates = m_diet_all, lm_trend = T, data = analysis),
  print_lm(exposure = "taste_diplos", outcome = "glu", label="0-2hr.Glucose ~ All Diet Patterns", 
           covariates = gsub("[+]fast_cat", "", m_diet_all), lm_trend = T, data = analysis %>% filter(fast_cat=="0to2hr")),
  print_lm(exposure = "taste_diplos", outcome = "hba1c", label="HbA1c ~ All Diet Patterns", covariates = m_diet_all, lm_trend = T, data = analysis)
