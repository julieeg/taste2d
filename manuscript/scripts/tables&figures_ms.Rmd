...
main_palette <- c(palettes$NatMainBackground2$Level4[2:1], palettes$NatMainBackground2$Level2[1], "grey80")  #palettes$nature_diplos_dominant[c(1,3:5)]
...
# color palette
haplo_palette <- main_palette[c(1:2,4)]
...
tab_gluXfast_diplocat <- make_pretty_lm(
  fread(paste0(taste2d_dir, outDir, "/tab_res_gluXfast_diplocat_basebmi.csv"))) %>%
  filter(Model == "BMI")

tab_gluXfast_diplocont <- make_pretty_lm(
  fread(paste0(taste2d_dir, outDir, "/tab_res_gluXfast_diplocont_basebmi.csv"))) %>%
  filter(Model == "BMI") %>%
...
diplo_palette <- main_palette#taste_palette
```
...
negcontrl_palette <- c(rs10772420_A=rgb2hex(109,165,103), rs2597979_G=rgb2hex(193,121,99),
                       rs713598_G=diplo_palette[[2]], rs1726866_G=diplo_palette[[2]], 
                       rs10246939_C=diplo_palette[[2]])
...
## NO significanat sex interactions in BMI-adjusted model
...
# Categorical diplotype 
make_pretty_lm(rbind.data.frame(make_pretty_lm(
  print_lm(exposure = "taste_diplos", outcome="glu2hr_mgdl", 
           covariates = gsub("[+]sex","", adj_models[[1]]), 
  ...
  fwrite(paste0(taste2d_dir, "../manuscript/output/tab_sens_glu_diplocat_sexstrat_bmi.csv"))

# Continuous diplotype 
make_pretty_lm(do.call(rbind.data.frame, lapply(c("Female","Male"), function(x) {
  print_lm(exposure = "taste_diplos.num", outcome="glu2hr_mgdl", 
           covariates = gsub("[+]sex","", adj_models[[1]]),
           label=paste0(x[[1]],"-BMI"), data=analysis %>% filter(sex == x))
}))) %>% fwrite(paste0(taste2d_dir, "../manuscript/output/tab_sens_glu_diplocont_sexstrat_bmi.csv"))


## Examine combined taster/supertaster group for females ---------------

rbind.data.frame(make_pretty_lm(
  print_lm(exposure = "taste_pav", outcome="glu2hr_mgdl", 
           covariates = gsub("[+]sex","", adj_models[[1]]),
           label="Female", data=analysis %>% filter(sex=="Female"))
  ), make_pretty_lm(
    print_lm(exposure = "taste_pav", outcome="glu2hr_mgdl", 
  ...
  fwrite(paste0(taste2d_dir, "../manuscript/output/tab_sens_glu_pav_sexstrat_bmi.csv"))
