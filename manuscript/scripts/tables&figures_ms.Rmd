...
outDir <- "../../archive/data/processed/manuscript/"
...
emm_glu_mgdl_bmi <- rbind.data.frame(
  read.csv(paste0("../../archive/data/processed/manuscript/tab_res_emm_glu_diplo_bmi.csv")) %>%
    mutate(fast = "Random", .after=model), 
  read.csv(paste0("../../archive/data/processed/manuscript/tab_res_emm_gluXfast_6to12_diplo_bmi.csv"))
) ; emm_glu_mgdl_bmi
...
panelA.supp <-  fread(paste0(outDir, "/tab_sens_behavcovars_diplo_bmi.csv"))  %>%
  arrange(beta) %>%
...
panelB.supp <- fread(paste0(outDir, "/tab_sens_glu2hr_behavcovars_bmi.csv")) %>%
  mutate(Diet=model) %>%
  mutate(logp=-log10(p)) %>%
...
ggarrange(panelA.supp, "", panelB.supp, widths=c(1.65, 0.05, 1.15),nrow=1, 
  ...
  ggsave(filename=paste0("../output/fig_suppl_behav_covars_R1.pdf"), height=3.5, width=8) 
...
covar_qual <- left_join(
  fread(paste0("../../archive/data/processed/manuscript/tab_res_glu_diplo_behav.csv")), 
  fread(paste0("../../archive/data/processed/manuscript/tab_res_behav_covars_bmi.csv")),
  by="Model")
...
behav_covars.labs <- readRDS(paste0("../../archive/data/processed/manuscript/behav_covars_labs.rda"))

# Read in associations
negcntrl_behav <- fread(paste0("../../archive/data/processed/manuscript/tab_sens_dietlife_negcntrl_snps_bmi.csv")) %>%
...
# Lod in data
emm_snps_glu2hr <- fread(paste0("../../archive/data/processed/manuscript/tab_sens_emm_gluXfast_tas2r38snps_bmi.csv"))
...
```{r}
## Build Graphical abstract plot
emm_glu_mgdl_bmi 
emm_glu_gt3hr <- fread("../../archive/data/processed/manuscript/tab_sens_emm_glugt3hr_bmi.csv")

# Create emm data frame
emm_glu_graphabs <- rbind.data.frame(emm_glu_mgdl_bmi %>% filter(fast == "0to2hr"), emm_glu_gt3hr)

## Make figure
pdf("../../archive/output/manuscript/fig_graphabs_v1.pdf", height=3, width=5)
emm_glu_graphabs %>% 
  mutate(fast=factor(fast, levels=rev(c("3+hr", "0to2hr")), labels=rev(c("3-12 hr\nsince last meal", "0-2 hr\nsince last meal\n (i.e.,'postprandial')"))),
         Diplotypes = factor(level, levels=c("AVI/AVI", "AVI/PAV", "PAV/PAV"))) %>%
  mutate(type = factor(ifelse(fast=="Anytime from\n0-12 hrs", "Random", "Fasting"), levels=c("Random", "Fasting"))) %>%
  ggplot(aes(y=emmean, x=fast, ymin=lowCI, ymax=upCI, color=Diplotypes)) + 
  #facet_grid(~type, scale="free", space="free_x", labeller = as_labeller(c("Random" = "Random Glucose", "Fasting"="Glucose by Self-Reported Fasting Time"))) +
  #facet_grid(~type, scale="free", space="free_x", labeller = as_labeller(c("Random" = "", "Fasting"=""))) +
  scale_y_continuous(limits=c(87.5, 90), breaks=seq(87.5,90,0.5)) +
  geom_point(size=2.75, position=position_dodge(0.25)) + 
  geom_errorbar(width=0.15, position=position_dodge(0.25), linewidth=0.65) + 
  scale_color_manual(values=taster_palette, name="**_TAS2R38_ Diplotype**") +
  ylab("Blood glucose level (mg/dL)\n") + xlab("Self-reported fasting time") + 
  theme_classic() +
  theme(
  axis.title.x = element_blank(), #element_text(size=12, vjust=-1), 
  axis.title.y = element_text(size=10, face="bold", vjust=-2), 
  axis.text.x = element_text(size=11,  color="black"), 
  axis.text.y = element_text(size=9),
  panel.grid.major.y = element_line(linewidth = 0.25),
  legend.position = "inside",
  legend.title = element_markdown(face="bold", size=10, margin = margin(0,0,2.5,0)), #element_text(face="bold", size=9),
  legend.text = element_text(size=10, face="italic"), #legend.title=element_blank(),
  legend.position.inside = c(0.85, 0.95),  # Moves legend to top-right inside plot
  legend.justification.inside = c(1, 1),  # Aligns legend's top-right corner
